...
public class PlayerImp extends UnicastRemoteObject implements Player {
    ...
    private Result[] result = new Result[2]; // r�sultats des 2 joueurs
    private int[] v = new int[2]; // nombre de victoires des 2 joueurs
    private int nbPlayers = 1; // nombre de joueurs identifi�s

    /**
     * @param dispenser cet objet est n�cessaire pour informer le serveur de la
     *                  n�cessit� de cr�er un nouvel objet Player
     */
     ...
     * @return id avec lequel le joueur peut r�aliser des actions
     */
    synchronized public int hello(String name) throws RemoteException, InterruptedException {
        if (this.name[0] == null && this.name[1] == null) {
            this.name[0] = name;
            System.out.println("Joueur identifié : " + name);
            wait();
            return 0;
    ...
    synchronized public Score play(int id, Action action) throws InterruptedException {
        ...
        if (turn < MAXTURNS) {
            ...
            System.out.println(name[0] + "(" + this.action[0] + ")" + " vs " + name[1] + "(" + this.action[1] + ")");
            result[0] = gagner(this.action[0], this.action[1]);
            result[1] = gagner(this.action[1], this.action[0]);
        } else {
            if (v[0] == v[1]) {
                resFP = Result.NUL;
                System.out.println("NUL -> Aucun gagnant");
            }
            if (v[0] <= v[1]) {
                if (id == 0) {
                    resFP = Result.PERDU;
                    ...
                    resFP = Result.GAGNE;
                    System.out.println("GAGNE -> " + name[1]);
                }
            ...
            if (v[0] >= v[1]) {
                if (id == 0) {
                    resFP = Result.GAGNE;
                    System.out.println("GAGNE -> " + name[0]);
                } else {
                    resFP = Result.PERDU;
                }
            }
